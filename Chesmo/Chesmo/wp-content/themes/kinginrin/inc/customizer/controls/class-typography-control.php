...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Kinginrin_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Kinginrin_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'kinginrin-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'kinginrin-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), KINGINRIN_VERSION, true );
			wp_enqueue_style( 'kinginrin-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), KINGINRIN_VERSION );

			wp_enqueue_script( 'kinginrin-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'kinginrin-typography-selectWoo' ), KINGINRIN_VERSION, true );
			wp_enqueue_style( 'kinginrin-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), KINGINRIN_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'kinginrin_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'kinginrin' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'kinginrin' );
			$this->json[ 'google_fonts' ] = apply_filters( 'kinginrin_typography_customize_list', kinginrin_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = kinginrin_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'kinginrin' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'kinginrin' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'kinginrin' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'kinginrin' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.family ) ) { #>
				<div class="kinginrin-font-family">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="kinginrin-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="kinginrin-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="kinginrin-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.weight ) ) { #>
				<div class="kinginrin-font-weight">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="kinginrin-font-transform">
					<label>
