( function( api ) {
	...
	api.controlConstructor['kinginrin-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.kinginrin-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the kinginrin_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'kinginrin_get_all_google_fonts_ajax',
								kinginrin_customize_nonce: kinginrin_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.kinginrin-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.kinginrin-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.kinginrin-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.kinginrin-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.kinginrin-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.kinginrin-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.kinginrin-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.kinginrin-font-variant' );
			...
			control.container.on( 'change', '.kinginrin-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.kinginrin-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.kinginrin-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.kinginrin-font-family select' ).selectWoo();
	jQuery( '.kinginrin-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".kinginrin-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.kinginrin-font-variant' ).hide();
		}
	});
