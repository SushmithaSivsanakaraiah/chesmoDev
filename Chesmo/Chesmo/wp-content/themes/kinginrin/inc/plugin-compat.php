...
add_action( 'after_setup_theme', 'kinginrin_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function kinginrin_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'kinginrin_construct_sidebars' );
}

if ( ! function_exists( 'kinginrin_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'kinginrin_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function kinginrin_woocommerce_start() { ?>
...
if ( ! function_exists( 'kinginrin_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'kinginrin_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function kinginrin_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'kinginrin_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kinginrin_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function kinginrin_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'kinginrin_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'kinginrin_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kinginrin_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function kinginrin_bbpress_css() {
...
if ( ! function_exists( 'kinginrin_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kinginrin_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function kinginrin_buddypress_css() {
...
if ( ! function_exists( 'kinginrin_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'kinginrin_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function kinginrin_beaver_builder_css() {
