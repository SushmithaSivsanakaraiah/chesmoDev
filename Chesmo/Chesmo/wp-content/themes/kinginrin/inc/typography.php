...
if ( ! function_exists( 'kinginrin_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'kinginrin_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function kinginrin_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', kinginrin_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $kinginrin_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $kinginrin_settings[$key], ':' ) !== false ) {
					$kinginrin_settings[$key] = current( explode( ':', $kinginrin_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $kinginrin_settings[$key] );

				// Grab the variants using the plain name
				$variants = kinginrin_get_google_font_variants( $kinginrin_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'kinginrin_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'kinginrin_fonts_subset','' );
		...
		$default_fonts = KINGINRIN_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('kinginrin-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('kinginrin-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'kinginrin_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'kinginrin_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function kinginrin_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'kinginrin_fonts_customize_register' ) ) {
			// Bail if Kinginrin Premium is active
			return;
		}
		...
		$defaults = kinginrin_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Kinginrin_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'kinginrin_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'kinginrin_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'kinginrin_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Kinginrin_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'kinginrin' ),
					'settings' => array(
						'family' => 'kinginrin_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'kinginrin_settings[body_font_weight]',
						'transform' => 'kinginrin_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_setting(
			'kinginrin_settings[body_font_size]',
			array(
		...
		$wp_customize->add_control(
			new Kinginrin_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'kinginrin' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'kinginrin_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'kinginrin_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'kinginrin_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Kinginrin_Range_Slider_Control(
				...
				array(
					'type' => 'kinginrin-range-slider',
					'description' => __( 'Line height', 'kinginrin' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'kinginrin_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'kinginrin_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Kinginrin_Range_Slider_Control(
				...
				array(
					'type' => 'kinginrin-range-slider',
					'description' => __( 'Paragraph margin', 'kinginrin' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'kinginrin_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'kinginrin_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function kinginrin_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'kinginrin_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'kinginrin_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'kinginrin_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_kinginrin_get_all_google_fonts_ajax', 'kinginrin_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function kinginrin_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'kinginrin_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'kinginrin_customize_nonce' ] ), 'kinginrin_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'kinginrin_customize_nonce', 'kinginrin_customize_nonce' );
		...
		$fonts = kinginrin_get_all_google_fonts();
...
if ( ! function_exists( 'kinginrin_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: kinginrin_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function kinginrin_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, kinginrin_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'kinginrin_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'kinginrin_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: kinginrin_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function kinginrin_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'kinginrin_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function kinginrin_get_font_family_css( $font, $settings, $default ) {
		$kinginrin_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'kinginrin_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $kinginrin_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'kinginrin_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . kinginrin_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'kinginrin_add_to_font_customizer_list' ) ) {
	add_filter( 'kinginrin_typography_customize_list', 'kinginrin_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function kinginrin_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = kinginrin_get_all_google_fonts();
		$select_fonts = kinginrin_get_all_google_fonts( apply_filters( 'kinginrin_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $kinginrin_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $kinginrin_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $kinginrin_settings[ $setting ], kinginrin_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $kinginrin_settings[ $setting ] ) ) ] = array(
				'name' => $kinginrin_settings[ $setting ],
		...
		if ( function_exists( 'kinginrin_secondary_nav_get_defaults' ) ) {
			...
			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], kinginrin_typography_default_fonts() ) ) {
